  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch hand signs
        const handSignsResponse = await axios.get(
          "http://localhost:3004/api/handsigns"
        );
        setHandSigns(handSignsResponse.data);

        // Fetch videos
        const videosResponse = await axios.get(
          "http://localhost:3004/api/videos"
        );
        setVideos(videosResponse.data);

        // Fetch progress
        const progressResponse = await axios.get(
          `http://localhost:3004/api/progress/${user.id}`
        );
    ...
    <div className="validation-container">
      ...
      {allVideosValidated ? (
        <div className="completion-message">
          ...
          <h2>All Done! 🎉</h2>
          <p>
            You have completed all video validations. Thank you for your
            contribution!
          </p>
        </div>
      ) : (
        <>
          <div className="video-section">
            ...
            <div className="video-player">
              {currentVideo && (
                <video
                  src={`http://localhost:3004/videos/test.mp4`}
                  controls
                  autoPlay
                  loop
                  muted
