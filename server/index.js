...
// CORS configuration using a dynamic origin function that allows all origins
const corsOptions = {
  origin: (origin, callback) => {
    // Allow requests with no origin (like mobile apps or curl)
    // or any origin provided by the client.
    callback(null, true);
  },
...
app.get("/api/handsigns", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load hand signs" });
  }
});
...
app.post("/api/submit", (req, res) => {
  try {
    ...
    const userResultsPath = path.join(
      resultsDir,
      `user_${userId}_results.json`
    );
    let userResults = [];
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to save validation" });
  }
});
